...
export default function GroupTable({ groupLetter, groupStandings }) {
  if (!groupStandings) return null;

  return (
    <div className="bg-zinc-950/50 border border-zinc-800 rounded-xl overflow-hidden shadow-xl">
      <div className="bg-gradient-to-r from-zinc-900 to-zinc-800 px-3 sm:px-4 py-2 sm:py-2.5 border-b border-zinc-800">
        ...
        <table className="w-full text-left border-collapse text-[10px] sm:text-xs">
          ...
          <tbody className="divide-y divide-zinc-900/40 font-medium text-zinc-300">
            {groupStandings.map((row, index) => {
              ...
              return (
                <tr key={row.teamId} className={`hover:bg-zinc-800/40 transition-colors ${rowHighlight}`}>
                  <td className="py-1.5 sm:py-2 px-2 sm:px-3 text-center text-zinc-400 font-bold">{index + 1}</td>
                  <td className="py-1.5 sm:py-2 px-1 sm:px-2 text-zinc-100 font-semibold truncate max-w-[100px] sm:max-w-[120px]">{row.name}</td>
